...
mod courier_tests {
    ...
    fn courier_relaxation_payload_is_installed_and_copy_failure_is_fatal() {
        ...
        // The activation hook was removed; the JSON is the only shipped artifact.
        assert!(
            !prefix.join("etc/conda/activate.d/retread-relaxations.sh").exists(),
            "no relaxation activation hook may be installed"
        );
        let payload = serde_json::to_string(&parsed).unwrap();
        assert!(payload.contains("typing-extensions"), "payload: {payload}");
        assert!(payload.contains(">=4.12,<5"), "payload: {payload}");
        ...
        assert!(
            !extracted.join("etc/conda/activate.d/retread-relaxations.sh").exists(),
            "the emitted .conda must not carry a relaxation activation hook"
        );
    ...
    fn courier_without_relaxation_injects_no_warning_payload() {
        ...
        assert!(
            !recipe.build.script.contains("retread-relaxations.sh"),
            "no build may install a relaxation activation hook \
             (the courier's own zzz-retread activate/deactivate guard is a \
             separate feature and IS still expected in this script)"
        );
        assert!(
            recipe
                .source
                ...
                .all(|source| !source.url.ends_with(RELAXATION_JSON_FILENAME)
                    && !source.url.ends_with("retread-relaxations.sh"))
        );
        ...
        assert!(!extracted
            .join("etc/conda/activate.d/retread-relaxations.sh")
            .exists());
        assert!(
            !extracted
