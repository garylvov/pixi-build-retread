...
fn verify_payload_installed(lock: &RetreadLock, prefix: &Path) -> Result<()> {
    ...
    // The producer recorded that the consuming workspace's own
    // `[pypi-dependencies]` closure owns these dists; the courier never
    // installed them and must never demand them. Without this exemption the
    // install record would be written and then immediately contradicted by
    // verify -- the reader half of the writer/reader pair.
    let declared_owned = declared_pypi_owned_names(lock);
    let missing: Vec<String> = missing_locked_wheels_from_installed(lock, &installed)
        ...
        .filter(|item| {
            ...
            name.is_none_or(|n| !editable_owned.contains(&n) && !declared_owned.contains(&n))
        })
        .collect();
    if !missing.is_empty() {
        bail!(
    ...
    for wheel in &lock.wheels {
        let name = normalize_dist_name(&wheel.name);
        if editable_owned.contains(&name) || declared_owned.contains(&name) {
            continue;
        }
...
fn installed_payload_libraries(
    ...
    let editable_owned = editable_owned_distributions(&site_packages);
    let conda_owned = conda_owned_distributions(&site_packages);
    // Declared-owned dists were never installed by the courier, so their
    // shared libraries are not the courier's payload to audit (and the
    // (name, version) lookup below would hard-error on them).
    let declared_owned = declared_pypi_owned_names(lock);
