...
async fn stage_relaxation_payload_in(
    ...
    Ok(StagedRelaxationPayload {
        source_urls: vec![file_url(&json_path)?],
        json_path,
    })
}
...
mod tests {
    ...
    /// The activation hook is gone; the JSON payload is now the only staged
    /// artifact, so IT is what must fail loudly when its rename cannot land.
    #[tokio::test]
    async fn rename_failure_is_fatal() {
        let staging_dir = test_dir("rename-failure");
        let json_destination = staging_dir.join(RELAXATION_JSON_FILENAME);
        std::fs::create_dir_all(&json_destination)
            .expect("create directory at final json destination");
        std::fs::write(json_destination.join("blocker"), b"not replaceable")
            .expect("make destination directory non-empty");
        let manifest = RelaxationManifest::new("example-pack", vec![record("numpy")])
            .expect("non-empty manifest");
        let error = stage_relaxation_payload_in(&staging_dir, &manifest)
            .await
            .expect_err("renaming the json over a directory must fail");
        assert!(
            format!("{error:#}").contains("atomically placing relaxation JSON"),
            "unexpected error: {error:#}"
        );
        let _ = std::fs::remove_dir_all(staging_dir);
    }

    /// Regression guard for the deletion itself: staging must produce exactly
    /// one artifact and never an `etc/conda/activate.d` shell hook.
    #[tokio::test]
    async fn staging_emits_json_only_and_no_activation_hook() {
        let staging_dir = test_dir("json-only");
        let manifest = RelaxationManifest::new("example-pack", vec![record("numpy")])
            .expect("non-empty manifest");
        let staged = stage_relaxation_payload_in(&staging_dir, &manifest)
            .await
            .expect("stage payload");
        assert_eq!(staged.source_urls.len(), 1, "only the JSON may be shipped");
        assert!(staged.json_path.exists());
        let shells: Vec<_> = std::fs::read_dir(&staging_dir)
            .expect("read staging dir")
            .filter_map(|e| e.ok())
            .map(|e| e.file_name().to_string_lossy().into_owned())
            .filter(|n| n.ends_with(".sh"))
            .collect();
        assert!(shells.is_empty(), "no shell hook may be staged: {shells:?}");
        let _ = std::fs::remove_dir_all(staging_dir);
    }
