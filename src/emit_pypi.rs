...
# Usage: pixi run -e <your-env> bash <this script>   (or add a [tasks] line)
# Re-run after recreating the env (pixi wipes site-packages). Requires uv.
set -euo pipefail
DIR="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd)"
PYTHON="${{CONDA_PREFIX:?activate a pixi env first (pixi run/pixi shell)}}/bin/python"
exec uv pip install   --python "$PYTHON"   --find-links "$DIR/wheels"   --overrides "$DIR/overrides.txt"   --prerelease=allow   "{bundle_name}-pypi=={version}"
"#
    )
}
