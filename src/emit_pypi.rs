...
/// The zero-manifest consumption path: a self-contained installer the
/// user invokes from any activated pixi env. Standalone uv reconciles
/// against the live conda env (it prefers installed dists for every
/// resolution node), takes the FULL override table as a file, and
/// accepts prereleases per-invocation -- all three things pixi's
/// manifest cannot do without machine-written bytes.
pub fn render_install_script(bundle_name: &str, version: &str, retread_version: &str) -> String {
    let dist_underscored = format!("{}_pypi", bundle_name.replace('-', "_"));
    format!(
        r#"#!/usr/bin/env bash
# Generated by pixi-build-retread {retread_version} (retread-blueprint, script sync).
# Installs the {bundle_name} blueprint into the ACTIVE pixi env's python.
#
# Idempotent and fast when the overlay is intact (a marker check), so it is
# safe to hook into [activation] scripts -- recommended, because pixi PRUNES
# pypi packages it did not lock whenever it reconciles an env that has its
# own [pypi-dependencies]; the activation hook self-heals after any prune or
# env recreate. Requires uv.
set -euo pipefail
DIR="$(cd "$(dirname "${{BASH_SOURCE[0]}}")" && pwd)"
PYTHON="${{CONDA_PREFIX:?activate a pixi env first (pixi run/pixi shell)}}/bin/python"
# Fast path (~ms, safe for activation hooks): the overlay's dist-info
# directory exists at the right version. Pure filesystem -- no python
# startup.
if compgen -G "$CONDA_PREFIX/lib/python*/site-packages/{dist_underscored}-{version}.dist-info" > /dev/null; then
  exit 0
fi
echo "retread blueprint: installing {bundle_name}-pypi=={version} into $CONDA_PREFIX" >&2
exec uv pip install \
  --python "$PYTHON" \
  --find-links "$DIR/wheels" \
  --overrides "$DIR/overrides.txt" \
  --prerelease=allow \
  "{bundle_name}-pypi=={version}"
"#
    )
}

/// The full override table as PEP 508 requirement lines for uv's
/// `--overrides` file -- one line per name, no manifest involvement.
pub fn render_overrides_file(overrides: &BTreeMap<String, String>) -> String {
    let mut out = String::new();
    for (name, value) in overrides {
        if value == "*" {
            out.push_str(name);
...
pub async fn emit(
    ...
    // v1.7.1 script sync (default): the blueprint is consumed via a
    // generated installer; the workspace manifest is NEVER written.
    if config.blueprint.is_on() && config.blueprint_sync == crate::config::BlueprintSync::Script {
        let script = render_install_script(bundle_name, bundle_version, env!("CARGO_PKG_VERSION"));
        ...
        tokio::fs::write(
            root.join("overrides.txt"),
            render_overrides_file(&emit_plan.overrides),
        )
        .await
        .context("writing overrides.txt")?;
        tracing::info!(
            script = %script_path.display(),
            "blueprint: installer written; add a [tasks] line like \
             `setup-{} = \"bash {}/retread-pypi/{}/install.sh\"` and run it \
             inside your env -- the workspace manifest is never modified",
            bundle_name,
            pack_rel,
            bundle_name,
        );
    ...
    // v1.6.1 fence sync (opt-in via retread-blueprint-sync = "fence",
    // and the non-blueprint emit-pypi table mode): the feature
    // definition lives in a fenced region retread keeps in sync; the
    // user references `<bundle>-pypi` from an environment once. In
    // script mode an existing fence is REMOVED so mode switches
    // converge.
    let fence_wanted =
        !config.blueprint.is_on() || config.blueprint_sync == crate::config::BlueprintSync::Fence;
    if let Some(workspace) = workspace_dir {
        let manifest_path = workspace.join("pixi.toml");
        match tokio::fs::read_to_string(&manifest_path).await {
            Ok(text) => match if fence_wanted {
                sync_workspace_block(&text, bundle_name, &snippet)
            } else {
                remove_workspace_block(&text, bundle_name)
            } {
                Ok(Some(updated)) => {
...
mod tests {
    ...
    #[test]
    fn install_script_and_overrides_file() {
        let s = render_install_script("isaac-pack", "4.5.22", "1.7.1");
        assert!(s.starts_with("#!/usr/bin/env bash"));
        assert!(s.contains("--find-links \"$DIR/wheels\""));
        assert!(s.contains("--overrides \"$DIR/overrides.txt\""));
        assert!(s.contains("--prerelease=allow"));
        assert!(s.contains("\"isaac-pack-pypi==4.5.22\""));
        let mut o = BTreeMap::new();
        o.insert("requests".to_string(), ">=2.32.3".to_string());
        o.insert("tinyobjloader".to_string(), "==2.0.0rc13".to_string());
        o.insert("loose".to_string(), "*".to_string());
        let f = render_overrides_file(&o);
        assert_eq!(f, "loose\nrequests>=2.32.3\ntinyobjloader==2.0.0rc13\n");
    }

    #[test]
    fn fence_removal_on_script_mode() {
