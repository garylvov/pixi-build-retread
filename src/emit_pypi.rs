...
pub fn render_snippet_blueprint(
    ...
    let feature = format!("{bundle_name}-pypi");
    // FINDING-C (v1.8.0): declaring `find-links` suppresses pixi/uv's
    // IMPLICIT default PyPI index, so index-origin entries (isaacsim @
    // pypi.nvidia.com, ...) and any PyPI transitive resolve to "not found".
    // Pin the default index back explicitly and add each entry's own index
    // as an extra, so the blueprint is self-contained -- the same
    // manifest-driven index chain retread uses for conda probes. (A
    // workspace that already declares its own [pypi-options] indexes merges
    // these; the explicit default is idempotent.)
    let default_norm = crate::handler::PUBLIC_PYPI.trim_end_matches('/');
    let mut extras: Vec<String> = entry_indexes
        .iter()
        .filter(|u| u.trim_end_matches('/') != default_norm)
        .cloned()
        .collect();
    extras.sort();
    extras.dedup();
    ...
    if !extras.is_empty() {
        let joined = extras
            ...
            .collect::<Vec<_>>()
            .join(", ");
        opts.push_str(&format!("extra-index-urls = [{joined}]\n"));
    }
    let mut out = format!(
...
/// Remove this bundle's fenced block from a workspace manifest (mode
/// switched to script sync). `None` when no fence exists.
pub fn remove_workspace_block(manifest_text: &str, bundle_name: &str) -> Result<Option<String>> {
    let open = fence_open(bundle_name);
    let close = fence_close(bundle_name);
    match (manifest_text.find(&open), manifest_text.find(&close)) {
        (None, None) => Ok(None),
        (Some(start), Some(close_at)) if close_at > start => {
            ...
            out.push_str(manifest_text[..start].trim_end_matches('\n'));
            out.push('\n');
            out.push_str(&manifest_text[end..]);
            Ok(Some(out))
        }
        _ => Err(anyhow::anyhow!(
            "corrupted emit-pypi fence for `{bundle_name}`; fix or delete it by hand"
        )),
    }
}
...
/// resolution (see [`render_snippet_blueprint`]). So every prerelease the
/// blueprint depends on needs a `"name" = "==<prerelease>"` override row
/// or `pixi lock` fails "pre-releases not enabled". Three disjoint
/// sources, each invisible to the others:
...
pub async fn emit(
    ...
    if config.blueprint_sync.is_some() {
        tracing::warn!(
            "`retread-blueprint-sync` is deprecated and ignored as of v1.8.0: the \
             install-script path was removed and fence sync is now the only path. \
             Remove the key (and any `install.sh` task/activation line) from your \
             manifest.",
        );
    }
    ...
    let snippet = if config.blueprint.is_on() {
        ...
        shipped += 1;
        let prerelease_pins =
            collect_prerelease_pins(&emit_plan.overrides, wheels, &emit_plan.ship, &entries);
        // FINDING-C: each entry's own index becomes an extra-index-url so the
        // blueprint resolves index-origin entries without the consumer having
        // to declare them (find-links otherwise suppresses the default index).
        let entry_indexes: Vec<String> = entries
            .iter()
            .filter_map(|(_, e, _)| e.index.clone())
            .collect();
    ...
    // v1.8.0 fence sync: the blueprint reaches the workspace ONLY through
    // a pixi-native fenced [feature.<bundle>-pypi] block. The generated
    // install script (install.sh + overrides.txt) was removed in v1.8.0:
    // fence is now the single path. emit-pypi always runs here (blueprint
    // and non-blueprint table mode alike), so the fence is always synced.
    // `remove_workspace_block` is retained for the future "pack stopped
    // emitting -> strip the stale fence" path.
    ...
    // v1.8.0 stale-lock guard: pixi's satisfiability string-compares the
    // find-links PATH and is blind to directory contents, so a stale
    // pixi.lock silently installs OLD/registry wheels over the freshly
    // (re)written 999retread-tagged ones -- reintroducing the exact import
    // breakage the blueprint exists to prevent. We cannot make pixi
    // re-check, so (1) write a diffable marker a pre-flight task can
    // compare, and (2) log a loud reminder on every blueprint build.
    if config.blueprint.is_on() {
        let mut names: Vec<String> = Vec::new();
        let mut rd = tokio::fs::read_dir(&wheels_dir)
            .await
            .with_context(|| format!("reading {}", wheels_dir.display()))?;
        ...
        names.sort();
        let marker = format!(
            "# pixi-build-retread {} blueprint marker for {bundle_name} {bundle_version}.\n\
             # Diff against the previous build to detect an out-of-date pixi.lock;\n\
             # pixi's lock check does NOT inspect find-links contents -- re-lock on change.\n{}\n",
            env!("CARGO_PKG_VERSION"),
            names.join("\n"),
        );
        tokio::fs::write(root.join("blueprint.lock"), marker)
            .await
            .context("writing blueprint.lock")?;
        tracing::warn!(
            bundle = %bundle_name,
            "blueprint (re)written -- run `pixi lock` before `pixi install`: pixi's \
             satisfiability is blind to find-links contents, so a stale lock silently \
             installs old/registry wheels instead of the retread-fixed ones",
        );
    }
    tracing::info!(
        bundle = %bundle_name,
...
mod tests {
    ...
    fn blueprint_snippet_is_static_and_tableless() {
        ...
        // FINDING-C: the default index is pinned explicitly (find-links would
        // otherwise suppress it); no entry indexes -> no extra-index-urls.
        assert!(s.contains("index-url = \"https://pypi.org/simple/\""));
        assert!(!s.contains("extra-index-urls"));
    ...
    #[test]
    fn blueprint_snippet_emits_entry_index_chain() {
        // FINDING-C: an index-origin entry (isaacsim @ pypi.nvidia.com) must
        // become an extra-index-url so the blueprint resolves it; the public
        // PyPI entry index is deduped against the explicit index-url.
        let s = render_snippet_blueprint(
            "isaac-pack",
            "isaac-pack",
            "6.0.0",
            ...
            &[
                ...
                "https://pypi.org/simple".to_string(), // == default (slash-normalized) -> dropped
                "https://pypi.nvidia.com".to_string(), // dup -> collapsed
            ],
            "1.8.0",
        );
        assert!(s.contains("index-url = \"https://pypi.org/simple/\""));
        assert!(s.contains("extra-index-urls = [\"https://pypi.nvidia.com\"]"));
    }
    ...
    #[test]
    fn fence_removal_converges() {
        let manifest = "[workspace]\nname = \"x\"\n";
        let with_fence = sync_workspace_block(manifest, "p", "[feature.p-pypi]\na = 1")
            .unwrap()
            .unwrap();
        let removed = remove_workspace_block(&with_fence, "p").unwrap().unwrap();
        assert!(!removed.contains("pixi-build-retread emit-pypi"));
        assert!(removed.contains("[workspace]"));
        // Idempotent: nothing to remove -> None.
        assert!(remove_workspace_block(&removed, "p").unwrap().is_none());
    }
